...
GRGroupsModule.prototype.load = function(cookies,successFn,failureFn){
	...
	*/
	...
	if(_part != 1) _part = 2;
	this._parse(JSON.stringify(_topBarData[ _part ]));
	successFn();
};
...
function addUsersGroups(){
	...
	// Phase Insertion
	var _href;
	if(window.location.href.match("group_settings.html")) _href = "home.html?part=2";
	else if(window.location.href.match("lists.html")) _href = "home.html?part=5";
	else _href = "home.html?part=1";

	// All Groups
	allMyGroups += '<li><a class="usergrouplinks" href="'+_href+'" ><span class="glyphicon glyphicon-stop" style="color:transparent;"></span>&nbsp;Show All Groups</a></li><hr />';
